...
const STATUS_CONFIG = {
  new: { label: 'New', color: 'bg-blue-100 text-blue-800', icon: <Bell className="h-4 w-4" /> },
  preparing: { label: 'Preparing', color: 'bg-orange-100 text-orange-800', icon: <Flame className="h-4 w-4" /> },
  ready: { label: 'Ready', color: 'bg-green-100 text-green-800', icon: <CheckCircle className="h-4 w-4" /> },
  served: { label: 'Served', color: 'bg-gray-100 text-gray-800', icon: <ChefHat className="h-4 w-4" /> },
};
...
const INITIAL_TICKETS: KitchenTicket[] = [
  {
    ...
    status: 'new',
    items: [
      { name: 'Classic Choripán', quantity: 2 },
      { name: 'Rustic Fries', quantity: 1 }
    ],
  ...
  },
  {
    ...
    status: 'preparing',
    items: [
      { name: 'Provoleta Choripán', quantity: 1 },
      { name: 'Beef Empanada', quantity: 3 }
    ],
...
export default function KdsContainer() {
  ...
  const updateTicketStatus = (ticketId: string, nextStatus: TicketStatus) => {
    ...
    toast({ title: 'Status Updated', description: `Ticket ${ticketId} is now ${nextStatus}.` });
  };
      ...
      <header className="bg-primary p-4 text-white flex justify-between items-center shadow-lg">
        ...
        <div className="flex gap-4">
          <Badge className="bg-white/20 text-white border-none text-lg px-4 py-1">
            Active: {tickets.filter(t => t.status !== 'served').length}
          </Badge>
          <div className="text-right">
            <div className="font-bold text-lg">Shift: Lunch</div>
            <div className="text-xs opacity-70">Kitchen Station 01</div>
          </div>
        </div>
      </header>
            ...
            <Card key={ticket.id} className={`overflow-hidden border-2 flex flex-col h-[400px] ${ticket.status === 'new' ? 'border-primary shadow-lg animate-pulse' : 'border-border'}`}>
              ...
              <CardFooter className="p-4 bg-muted/30 grid grid-cols-2 gap-2 mt-auto">
                ...
                {ticket.status === 'served' && (
                  <Badge className="col-span-2 justify-center py-2 text-md">COMPLETED</Badge>
                )}
              </CardFooter>
            </Card>
          ))}
